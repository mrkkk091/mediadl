...
DIRECT_VIDEO_EXT = (".mp4", ".m4v", ".mov", ".webm", ".mkv", ".avi", ".wmv", ".flv", ".3gp", ".ts")
...
def platform_of(url):
    ...
    if re.search(r"(^|\.)(facebook\.com|fb\.com|fb\.watch)$", host):
        return "facebook"
    if "youtu" in u:
        return "youtube"
    return "other"
...
PLATFORM_DIR = {"tiktok": "TikTok", "instagram": "Instagram", "facebook": "Facebook",
...
def music_spotify(url):
    ...
    if single:
        ...
        if kind == "playlist" and total >= 50:
            say("Note: Spotify's public page may not list every track of a large playlist.", "y")
    say(f"Saving to: {outdir}", "y")
...
DIRECT_VIDEO_EXT_ALL = DIRECT_VIDEO_EXT + (".m3u8", ".mpd")
META_TAG_RE = re.compile(r"<meta\b[^>]*>", re.I)
TAG_ATTR_RE = re.compile(r"""(?:([a-zA-Z][\w:-]*)\s*=\s*"([^"]*)"|([a-zA-Z][\w:-]*)\s*=\s*'([^']*)')""")
VIDEO_SRC_RE = re.compile(r'<(?:video|source)\b[^>]*\bsrc\s*=\s*["\']([^"\']+)["\']', re.I)
JSON_URL_RE = re.compile(
    r'"(?:file|src|url|hls|dash|manifest|source|videoUrl|video_url|contentUrl|playUrl)"\s*:\s*'
    r'"([^"]+\.(?:m3u8|mpd|mp4|webm)(?:\?[^"]*)?)"', re.I)
BARE_URL_RE = re.compile(r'https?://[^\s"\'<>]+?\.(?:m3u8|mpd|mp4)(?:[^\s"\'<>]*)', re.I)
IFRAME_RE = re.compile(r'<iframe\b[^>]*\bsrc\s*=\s*["\']([^"\']+)["\']', re.I)
TITLE_TAG_RE = re.compile(r"<title[^>]*>([^<]{1,200})</title>", re.I)
...
def _tag_attrs(tag):
    attrs = {}
    for m in TAG_ATTR_RE.finditer(tag):
        if m.group(1):
...
def page_title(html):
    ...
    return _txt(m.group(1))[:80] if m else ""


def sniff_video_urls(html, base_url):
    """Scan a page's HTML for video files, HLS streams, or embedded players -
    the same signals a browser or a video-saving app would look for."""
    text = html.replace("\\/", "/")
    candidates, embeds, seen = [], [], set()

    def add(raw, kind=None):
        u = (raw or "").strip().replace("&amp;", "&")
        if not u or u.startswith(("data:", "blob:")):
            return
        u = urllib.parse.urljoin(base_url, u)
        if u in seen or len(u) > 2000:
            return
        seen.add(u)
        candidates.append((kind or _kind_of(u), u))
    ...
    for m in IFRAME_RE.finditer(text):
        ...
        if any(host == h or host.endswith("." + h) for h in EMBED_HOSTS):
            embeds.append(u)

    kind_rank = {"file": 0, "hls": 1, "dash": 2}
    candidates.sort(key=lambda c: (kind_rank.get(c[0], 3), not re.search(r"1080|720", c[1])))
...
def fetch_page_html(url, referer=None):
    ...
    with open_url(urllib.request.Request(url, headers=headers), timeout=20) as r:
        ...
        raw = r.read(MAX_PAGE_BYTES + 1)[:MAX_PAGE_BYTES]
    try:
        return raw.decode("utf-8"), final_url
    except UnicodeDecodeError:
        return raw.decode("latin-1", "replace"), final_url


def _probe_url_kind(url, referer):
    """HEAD (falling back to a 1-byte GET) a URL to see what it actually is."""
    ...
    if "mpegurl" in ctype:
        return "hls", final_url, resp
    if "dash+xml" in ctype:
        return "dash", final_url, resp
    if ctype.startswith("video/"):
        return "file", final_url, resp
    if ctype in ("", "application/octet-stream", "binary/octet-stream") and path.endswith(DIRECT_VIDEO_EXT):
        return "file", final_url, resp
    if ctype.startswith("text/html") or ctype == "":
        return "page", final_url, resp
    return None, final_url, resp


def stream_download_file(url, outdir, referer, name_hint=None):
    """Download a URL that points straight at a video file."""
    dest = None
    try:
        headers = {"User-Agent": UA, "Referer": referer} if referer else {"User-Agent": UA}
        ext = os.path.splitext(urllib.parse.urlsplit(url).path)[1].lower()
        if ext not in DIRECT_VIDEO_EXT:
            ext = ".mp4"
        name = clean(name_hint or os.path.splitext(os.path.basename(
            urllib.parse.urlsplit(url).path))[0]) or "video"
        ...
        if os.path.getsize(dest) < 2048:
            raise ValueError("response was too small to be a real video")
        print()
        say(f"Saved: {dest}", "g")
        return True
    except JobCancelled:
        ...
        if dest and os.path.exists(dest):
            try:
                ...
                pass
        say(f"Direct download failed: {e}", "r")
        return False


def hls_download(m3u8_url, outdir, referer, name_hint=None):
    """Download an HLS (.m3u8) stream by remuxing it with ffmpeg - the same
    kind of stream many news sites and clip sites serve their video as."""
    if not shutil.which("ffmpeg"):
        say("This video needs ffmpeg, which is missing.", "r")
        return False
    os.makedirs(outdir, exist_ok=True)
    ...
    ok = run(base_cmd + ["-c", "copy", dest])
    if not ok or not os.path.exists(dest) or os.path.getsize(dest) < 2048:
        if os.path.exists(dest):
            try:
                os.remove(dest)
            except OSError:
                pass
        check_cancel()
        ok = run(base_cmd + ["-c:v", "copy", "-c:a", "aac", "-b:a", "128k", dest])
    if not ok or not os.path.exists(dest) or os.path.getsize(dest) < 2048:
        if os.path.exists(dest):
            try:
                os.remove(dest)
                ...
                pass
        say("Could not download that video stream.", "r")
        return False
    say(f"Saved: {dest}", "g")
    return True


def dash_download(mpd_url, outdir, referer, name_hint=None):
    """Download a DASH (.mpd) stream by remuxing it with ffmpeg - the other
    common way long-form video is split into segments on the web."""
    if not shutil.which("ffmpeg"):
        say("This video needs ffmpeg, which is missing.", "r")
        return False
    os.makedirs(outdir, exist_ok=True)
...
def find_page_videos(url):
    """Look at a link and report every video-like thing on it, without
    downloading anything. Returns a dict: direct (a single {"kind","url"} if
    the link itself is already a video/stream, else None), items (candidates
    found on the page, best first, embeds last), referer, and title."""
    origin = f"{urllib.parse.urlsplit(url).scheme}://{urllib.parse.urlsplit(url).netloc}/"
    kind, final_url, _ = _probe_url_kind(url, origin)
    if kind in ("file", "hls", "dash"):
        return {"direct": {"kind": kind, "url": final_url}, "items": [],
                "referer": origin, "title": ""}
    html, page_url = fetch_page_html(url)
    if html is None:
        return {"direct": None, "items": [], "referer": origin, "title": ""}
    name_hint = page_title(html)
    candidates, embeds = sniff_video_urls(html, page_url)
    items = [{"kind": k, "url": u} for k, u in candidates[:15]]
    items += [{"kind": "embed", "url": u} for u in embeds[:5]]
    return {"direct": None, "items": items, "referer": page_url, "title": name_hint}


def video_label(item):
    kind, u = item["kind"], item["url"]
    if kind == "embed":
        host = (urllib.parse.urlsplit(u).hostname or "").lower()
        if host.startswith("www."):
            host = host[4:]
        return f"Embedded player - {host}" if host else "Embedded player"
    name = os.path.basename(urllib.parse.urlsplit(u).path) or u
    if len(name) > 60:
        name = name[:57] + "..."
    tag = {"file": "video file", "hls": "HLS stream", "dash": "DASH stream"}.get(kind, kind)
    return f"{name}  ({tag})"


def download_chosen_item(item, outdir, referer, name_hint, height=None, _depth=0):
    """Download one specific item that find_page_videos() reported."""
    kind, u = item["kind"], item["url"]
    if kind == "embed":
        return download_video(u, height, _depth=_depth + 1)
    real_kind, final_c, _ = _probe_url_kind(u, referer)
    real_kind = real_kind or kind
    if real_kind == "file":
        return stream_download_file(final_c, outdir, referer, name_hint)
    if real_kind == "hls":
        return hls_download(final_c, outdir, referer, name_hint)
    if real_kind == "dash":
        return dash_download(final_c, outdir, referer, name_hint)
    return False


def generic_any_site_download(url, outdir, height=None, _depth=0):
    """Automatic last resort for a site yt-dlp doesn't recognize: check if the
    link is itself a video, and if not, fetch the page and try whatever it
    finds, best candidate first, until one works."""
    say("Reading the page for a video...", "y")
    try:
        info = find_page_videos(url)
    except Exception as e:
        say(f"Could not read that page: {e}", "r")
        return False
    if info["direct"]:
        return download_chosen_item(info["direct"], outdir, info["referer"], "", height, _depth)
    items = [it for it in info["items"] if it["kind"] != "embed" or _depth < 1]
    if not items:
        return False
    for item in items:
        check_cancel()
        if item["kind"] == "embed":
            say("Found an embedded video player, trying that...", "y")
        if download_chosen_item(item, outdir, info["referer"], info["title"], height, _depth):
            return True
    return False


def preview_and_choose(url, outdir, height=None, _depth=0):
    """Interactive version for the terminal menu: show what was found on the
    page and let the person pick one (or all), instead of auto-picking."""
    say("Looking for videos on that page...", "y")
    try:
        info = find_page_videos(url)
    except Exception as e:
        say(f"Could not read that page: {e}", "r")
    ...
    items = [it for it in info["items"] if it["kind"] != "embed" or _depth < 1]
    if not items:
        say("No videos found on that page.", "r")
        return False
    if len(items) == 1:
        chosen = items
    else:
        say(f"\nFound {len(items)} video(s) on that page:", "b")
        for i, it in enumerate(items, 1):
            print(f"  {i}) {video_label(it)}")
        pick = ask("Choose a number [1] (Enter = first, 'a' = download all):", "1").strip().lower()
        if pick == "a":
            chosen = items
    ...
    for i, item in enumerate(chosen, 1):
        check_cancel()
        if len(chosen) > 1:
            say(f"\n[{i}/{len(chosen)}] {video_label(item)}", "b")
        if download_chosen_item(item, outdir, info["referer"], info["title"], height, _depth):
            ok_count += 1
    if len(chosen) > 1:
        say(f"\nFinished: {ok_count}/{len(chosen)} downloaded.", "g")
    return ok_count > 0
...
def download_video(url, height=None, _depth=0, interactive=False):
    ...
    if plat == "other":
        origin = f"{urllib.parse.urlsplit(url).scheme}://{urllib.parse.urlsplit(url).netloc}/"
        cmd += ["--add-header", f"Referer:{origin}"]
    ...
    if playlist:
        ...
        cmd += ["--yes-playlist", "-P", outdir,
                "-o", "%(playlist_index)02d - %(title)s.%(ext)s"]
    elif plat in ("tiktok", "instagram", "facebook"):
    ...
    if not ok and plat == "facebook":
        if find_spec("curl_cffi"):
            say("Trying Facebook with browser impersonation...", "y")
            ok = run_yt(cmd + ["--impersonate", "chrome"], url)
        if not ok:
            cleanup_orphans(outdir, before)
            say("Facebook download failed. Make sure the Reel is public, update "
                "yt-dlp (menu 9), or set a cookies.txt file in Settings.", "y")
    if not ok and plat == "other":
        cleanup_orphans(outdir, before)
        if interactive:
            ok = preview_and_choose(url, outdir, height, _depth=_depth)
        else:
...
def menu_video():
    say("\n== Video download ==  (YouTube, TikTok, Instagram, Facebook, or any other website)")
    say("TikTok = no watermark. Instagram/Facebook = original quality (public only). "
        "Other sites: works as long as the page has a playable video.", "y")
    url = ask_url()
    if not url:
        return
    height = None
    if platform_of(url) in ("youtube", "facebook", "other"):
        print("Quality: 1) Best  2) 1080p  3) 720p  4) 480p")
        height = {"2": 1080, "3": 720, "4": 480}.get(ask("Choose [1]:", "1"))
    download_video(url, height, interactive=True)
...
def build_job(body):
    kind = body.get("kind")
    if kind == "page_item":
        return build_page_item_job(body)
    if kind == "gallery":
...
def build_page_item_job(body):
    item_url = body.get("item_url")
    item_kind = body.get("item_kind")
    if item_kind not in ("file", "hls", "dash", "embed"):
        raise ValueError("Unknown item type.")
    if not valid_url(item_url):
        raise ValueError("Invalid item link.")
    referer = body.get("referer")
    referer = referer.strip() if isinstance(referer, str) and valid_url(referer) else None
    title = str(body.get("title") or "")[:120]
    item = {"kind": item_kind, "url": item_url.strip()}
    outdir = os.path.join(cfg["video_dir"], PLATFORM_DIR["other"])
    label = title or video_label(item)
    return "video", label, lambda: download_chosen_item(item, outdir, referer, title), {}
    ...
    def _route_get(self, path, qs):
        ...
        if path == "/api/pagevideos":
            ...
            try:
                ...
                raise ValueError(f"Could not read that page: {e}")
            if info["direct"]:
                items = [dict(info["direct"], label=video_label(info["direct"]),
                              referer=info["referer"])]
            else:
                items = [dict(it, label=video_label(it), referer=info["referer"])
                         for it in info["items"]]
            self._json({"items": items, "title": info["title"]})
            return
...
<button class="feat glass f-video" data-open="video" data-r="26"><span class="badge"><svg class="ic"><use href="#i-video"/></svg></span><span class="ft"><b>Video</b><small>Any website with a video</small></span><svg class="ic chev"><use href="#i-chev"/></svg></button>
...
<p class="hint">YouTube, TikTok (no watermark), Instagram, Facebook Reels and any other site with a playable video. Facebook and Instagram work for public videos.</p>
...
<p class="hint">Page with more than one video, like a gallery or a preview grid? <button class="btn soft sm" id="goFindVideos" type="button">Find videos on this page</button></p>
<div id="videoFindOut"></div>
</div>
...
function startPageItem(it,pageTitle,btn){
  return startJob({kind:'page_item',item_url:it.url,item_kind:it.kind,referer:it.referer,title:pageTitle||''},btn);
}
$('#goFindVideos').addEventListener('click',function(){
  var url=val('#videoUrl'),box=$('#videoFindOut'),btn=this;
  if(!url){toast('Paste a link first',true);return;}
  box.textContent='';box.appendChild(el('p','hint','Looking for videos on that page...'));
  busy(btn,api('/api/pagevideos?url='+encodeURIComponent(url)).then(function(d){
    box.textContent='';
    if(!d.items.length){box.appendChild(el('p','hint','No videos found on that page.'));return;}
    if(d.items.length>1){
      var allRow=el('div','res');
      var t=el('div','t');t.appendChild(el('b','',d.items.length+' videos found'));
      t.appendChild(el('span','','Download each one separately, or all at once'));
      var allBtn=el('button','btn sm','Download all');
      allBtn.addEventListener('click',function(){
        allBtn.disabled=true;
        d.items.forEach(function(it){startPageItem(it,d.title,el('button'));});
        toast('Started '+d.items.length+' downloads');
        setTimeout(function(){allBtn.disabled=false;},1200);
      });
      allRow.appendChild(t);allRow.appendChild(allBtn);
      box.appendChild(allRow);
    }
    d.items.forEach(function(it){
      var row=el('div','res');
      var t=el('div','t');t.appendChild(el('b','',it.label));
      var b=el('button','btn sm','Download');
      b.addEventListener('click',function(){startPageItem(it,d.title,b);});
...
MENU = [
    ...
    ("2", "Video download (any website with a video)", menu_video),
