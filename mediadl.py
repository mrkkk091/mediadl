...
def generic_any_site_download(url, outdir, height=None, _depth=0):
    """Last resort for a site yt-dlp doesn't recognize: check if the link is
    itself a video, and if not, fetch the page and look for one the way a
    video-saving app would - meta tags, <video> tags, player scripts, HLS/DASH
    streams, and embedded YouTube/Vimeo/Dailymotion players."""
    origin = f"{urllib.parse.urlsplit(url).scheme}://{urllib.parse.urlsplit(url).netloc}/"
    kind, final_url, _ = _probe_url_kind(url, origin)
    if kind == "file":
        return stream_download_file(final_url, outdir, origin)
    if kind == "hls":
        return hls_download(final_url, outdir, origin)
    if kind == "dash":
        return dash_download(final_url, outdir, origin)
    ...
    try:
        ...
        return False
    if html is None:
        say("That link isn't a web page or a video file.", "r")
        return False

    name_hint = page_title(html)
    candidates, embeds = sniff_video_urls(html, page_url)
    for cand_kind, cand_url in candidates[:20]:
        check_cancel()
        real_kind, final_c, _ = _probe_url_kind(cand_url, page_url)
        real_kind = real_kind or cand_kind
        if real_kind == "file" and stream_download_file(final_c, outdir, page_url, name_hint):
            return True
        if real_kind == "hls" and hls_download(final_c, outdir, page_url, name_hint):
            return True
        if real_kind == "dash" and dash_download(final_c, outdir, page_url, name_hint):
            return True

    if embeds and _depth < 1:
        say("Found an embedded video player, trying that...", "y")
        return download_video(embeds[0], height, _depth=_depth + 1)
    return False
...
def download_video(url, height=None, _depth=0):
