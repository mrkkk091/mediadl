...
- **Any other website:** for sites without a name-specific extractor, MediaDL first tries yt-dlp's generic mode. If that fails, it fetches the page itself and looks for a video the way a browser or a video-saving app would: `og:video` meta tags, `<video>`/`<source>` tags, common player scripts (JWPlayer-style `file`/`src`/`hls`/`dash` fields), and streamed video split into segments - both HLS (`.m3u8`) and DASH (`.mpd`) - which it reassembles with ffmpeg. If the page only has an embedded YouTube, Vimeo or Dailymotion player, it follows that embed and downloads from there instead (one level deep).\n\n  **What this can't do:** it reads the page's HTML, it doesn't run the page like a real browser does. So it can't see video that only appears after solving a challenge, logging in, or running heavy JavaScript, and it can't touch DRM-protected video - the same limits every downloader, Play Store apps included, run into. If your own phone browser can play a video MediaDL can't find, open the browser's "page info" / share option, or its developer tools network tab, to find the direct video link, then paste that link into MediaDL - a direct `.mp4`, `.m3u8` or `.mpd` link always works.
