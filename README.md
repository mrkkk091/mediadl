...
- **Video** from YouTube, TikTok (no watermark), Instagram, Facebook (Reels and videos), and **any other website with a playable video** (yt-dlp supports 1800+ sites by name, plus a generic mode and a direct-file fallback for the rest)
  - Best quality, 1080p, 720p or 480p
  - Playlist support
- **Search by name** - type a song name, pick a result, download it
- **Choose which video to download** - when a link leads to a page with more than one video (a gallery or a preview grid), MediaDL lists what it found so you can pick one, a few, or all of them, instead of guessing
...
| 2 | Video download | YouTube, TikTok, Instagram, Facebook, or any other site with a video |
...
| Videos | `/storage/emulated/0/Movies/<YouTube, TikTok, Instagram, Facebook or Other>/` |
...
| Facebook Reel fails | Make sure the Reel is public, update yt-dlp (menu **9**), or set a `cookies.txt` file in Settings (option 4) |
...
- **Spotify:** MediaDL does not download audio from Spotify. It reads the track list from Spotify's public embed page, searches YouTube for each song, picks the best match (mainly by song length, then artist and title), and tags the file with Spotify's title, artist, album and cover. The audio therefore comes from YouTube and may not be the exact Spotify version. Very large playlists may not be listed in full on Spotify's public page, so some songs can be missing. This feature depends on Spotify's page format, which can change.
- **TikTok:** the watermark-free version is used when available. A backup method is tried if the first one fails.
- **Facebook:** works for public Reels and videos (links like `facebook.com/reel/...` or `fb.watch/...`). Private videos need a `cookies.txt` from a logged-in browser. Facebook changes its pages often, so downloads can break until yt-dlp is updated. If `curl_cffi` is installed, a second attempt with browser impersonation is made automatically.
- **Any other website:** for sites without a name-specific extractor, MediaDL first tries yt-dlp's generic mode. If that fails, it fetches the page itself and looks for a video the way a browser or a video-saving app would: `og:video` meta tags, `<video>`/`<source>` tags, common player scripts (JWPlayer-style `file`/`src`/`hls`/`dash` fields), and streamed video split into segments - both HLS (`.m3u8`) and DASH (`.mpd`) - which it reassembles with ffmpeg. If the page only has an embedded YouTube, Vimeo or Dailymotion player, it follows that embed and downloads from there instead (one level deep). If a `cookies.txt` is set in Settings (option 8 -> 4), every step of this - reading the page, checking a link, and downloading the file or stream - sends those cookies too, so pages that need you to be logged in can work the same way they do in your own browser.

  **What this can't do:** cookies only help when the site's problem is "you're not logged in" - they can't get past a JavaScript challenge (like Cloudflare's "checking your browser" page), a CAPTCHA, or DRM-protected video, because MediaDL reads the page's HTML rather than running it like a real browser does. These are the same walls every downloader, Play Store apps included, run into. If your own phone browser can play a video MediaDL can't find, open the browser's "page info" / share option, or its developer tools network tab, to find the direct video link, then paste that link into MediaDL - a direct `.mp4`, `.m3u8` or `.mpd` link always works.

  **When a page has more than one video** - a gallery, a preview grid, several clips in one article - MediaDL doesn't just grab the first one it sees:
  - **In the menu:** after entering the link, if the page has more than one video you'll see a numbered list (what kind it is - video file, HLS stream, DASH stream, or an embedded player - and its file name). Type a number to download just that one, press Enter for the first, or type `a` to download all of them.
  - **In the web interface:** open the Video window, paste the link, and tap **Find videos on this page** instead of Download. Every video found is listed with its own Download button, plus a Download all button when there's more than one.
...
- **User responsibility.** You alone are responsible for what you download and how you use it. You must follow the copyright laws of your country and the terms of service of every website you use, including YouTube, Spotify, TikTok, Instagram, Facebook and SoundCloud. Downloading copyrighted material without permission may be illegal.
- **No affiliation.** MediaDL is an independent project. It is not affiliated with, endorsed by or sponsored by YouTube, Google, Spotify, TikTok, Instagram, Facebook, Meta, SoundCloud or any other service. All names and trademarks belong to their owners.
